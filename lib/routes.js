...
function getStaticRoutes(router) {
  ...
  return flattenRoutes(router).filter(({ url, component }) => {
    let options = extractComponentOptions(component);
    return !url.includes(':') && !url.includes('*') && (typeof options === 'undefined' || !options.exclude);
  })
}
